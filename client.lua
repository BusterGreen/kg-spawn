...
RegisterNetEvent('kg-spawn:client:openUI', function(value)
...
RegisterNetEvent('kg-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        KGCore.Functions.TriggerCallback('kg-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
                    myHouses[#myHouses+1] = {
...
RegisterNUICallback('setCam', function(data, cb)
    ...
    elseif type == "normal" then
        SetCam(KG.Spawns[location].coords)
...
RegisterNUICallback('chooseAppa', function(data, cb)
    ...
    TriggerServerEvent('KGCore:Server:OnPlayerLoaded')
    TriggerEvent('KGCore:Client:OnPlayerLoaded')
...
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    if type == "current" then
        PreSpawnPlayer()
        KGCore.Functions.GetPlayerData(function(pd)
            ped = PlayerPedId()
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('kg-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('kg-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('KGCore:Server:OnPlayerLoaded')
        TriggerEvent('KGCore:Client:OnPlayerLoaded')
        PostSpawnPlayer()
    elseif type == "house" then
        PreSpawnPlayer()
        TriggerEvent('kg-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('KGCore:Server:OnPlayerLoaded')
        TriggerEvent('KGCore:Client:OnPlayerLoaded')
        TriggerServerEvent('kg-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('kg-apartments:server:SetInsideMeta', 0, 0, false)
        PostSpawnPlayer()
    elseif type == "normal" then
        local pos = KG.Spawns[location].coords
        PreSpawnPlayer()
